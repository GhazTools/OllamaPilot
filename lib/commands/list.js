...
export default async function list() {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    if (info) {
      ...
      // Build stats line with dual tok/s metrics
      const statsparts = [
        `${CYAN}${info.type}${RESET}`,
        `${MAGENTA}${info.params}${RESET}`
      ];
      
      // Support both old and new metric formats
      if (info.promptTokPerSec && info.genTokPerSec) {
        statsparts.push(`${YELLOW}${SYM.bolt} ${info.promptTokPerSec}→${info.genTokPerSec} tok/s${RESET}`);
      } else if (info.tokPerSec) {
        // Legacy format fallback
        statsparts.push(`${YELLOW}${SYM.bolt} ${info.tokPerSec} tok/s${RESET}`);
      }
      
      console.log(`      ${statsparts.join(` ${DIM}${SYM.dot}${RESET} `)}`);
